    ...
    for event in pygame.event.get():
        ...
        if state == "menu":
            if event.type == pygame.MOUSEBUTTONDOWN:
                ...
                if UI_manager.start_rect.collidepoint(mouse_x, mouse_y):
                    state = "select"
                elif UI_manager.quit_rect.collidepoint(mouse_x, mouse_y):
                    running = False
                elif UI_manager.create_profile_button_rect.collidepoint(mouse_x, mouse_y):
                ...
                elif UI_manager.delete_profile_button_rect.collidepoint(mouse_x, mouse_y):
                    state = "delete"
                elif UI_manager.rankings_button_rect.collidepoint(mouse_x, mouse_y):
                    state = "rankings"

        elif state == "create":
            if refresh_create_screen:
                UI_manager.render_create_screen(input_text)  # Render screen only once on entry
            ...
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_RETURN:
                    ...
                    UI_manager.render_create_screen(input_text)  # Re-render the screen only when text changes
                else:
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                mouse_x, mouse_y = event.pos
                if UI_manager.back_button_rect.collidepoint(mouse_x, mouse_y):
                    state = "menu"
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                ...
                for i, profile_name in enumerate(profile_manager.profiles["profiles"].keys()):
                    ...
                    if profile_button_rect.collidepoint(mouse_x, mouse_y):
                        ...
                        break
            # Handle back button
            elif UI_manager.back_button_rect.collidepoint(mouse_x, mouse_y):
                state = "menu"
    ...
    if state == "menu":
        ...
        if not game_paused:
            ...
            if control:
                ...
                pygame.draw.rect(window, GRAY, UI_manager.pause_button_rect)
                pause_text = FONT.render("||", True, BLACK)
                window.blit(pause_text, UI_manager.pause_button_rect)

                # Handle pause
                current_time = time.time()
                if current_time - game_start_time > 10 and (pause_resume_time is None or current_time - pause_resume_time > 10):
                    if UI_manager.pause_button_rect.collidepoint(x1, y1) or UI_manager.pause_button_rect.collidepoint(x2, y2):
                        state = "pause"
                        game_paused = True
